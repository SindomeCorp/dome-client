...
## Testing
- Write new unit and integration tests for new features where applicable, using Node's built-in test runner and ECMAScript modules.
...
- Run `npm run test` to validate changes work before committing (and run `npm run lint` as part of pre-commit checks). Coverage must stay at or above 80%.
